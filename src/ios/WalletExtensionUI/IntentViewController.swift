...
class IntentViewController: UIViewController, D1IssuerProvisioningExtensionAuthorizationProviding  {
    ...
    override func viewDidLoad() {
        ...
        self.authButton.backgroundColor = UIColor(red: 203/255, green: 176/255, blue: 0/255, alpha: 1.0)
        ...
        if let image = UIImage(named: "ahliislamic_logo.png") {
    ...
    private func getJWTToken() {
        ...
        let urlString = "https://mye-bankonline.ahlibank.om/Islamic_Common_MBL/rest/Thales/GetThalesJWTToken?ConsumerID=\(consumerID)"
        
        guard let url = URL(string: urlString) else {
            NSLog("Outsystems=====> Invalid URL")
            self.activityIndicator.stopAnimating()
            self.completionHandler?(.canceled)
            return
        }
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            // Handle error
            if let error = error {
                NSLog("Outsystems=====> Rest API Error: \(error.localizedDescription)")
                self.activityIndicator.stopAnimating()
                self.completionHandler?(.canceled)
                return
            }
            ...
            guard let data = data else {
                NSLog("Outsystems=====> No data received")
                self.activityIndicator.stopAnimating()
                self.completionHandler?(.canceled)
                return
            }
            ...
            do {
                if let json = try JSONSerialization.jsonObject(with: data) as? [String: Any],
                   let jwtToken = json["AccessToken"] as? String {
                    NSLog("Outsystems=====> AccessToken: \(jwtToken)")
                    self.loginD1SDK(jwtToken)
                } else {
                    NSLog("Outsystems=====> AccessToken not found or invalid format")
                    self.activityIndicator.stopAnimating()
                    self.completionHandler?(.canceled)
                }
            } catch {
                NSLog("Outsystems=====> Parsing Error: \(error.localizedDescription)")
                self.activityIndicator.stopAnimating()
                self.completionHandler?(.canceled)
            }
        }
    ...
    private func loadFromKeychain(key: String) -> String? {
        let getQuery: [String: Any] = [kSecClass as String: kSecClassGenericPassword,
                                       ...
                                       kSecAttrAccessGroup as String: "group.com.ahliislamic.om",
